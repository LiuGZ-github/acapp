...
class Player extends AcGameObject
{
    ...
    shoot_fireball(tx, ty)
    {
        ...
        let speed = this.playground.height * 1.35;
    ...
    update()
    {
        ...
        if(Math.random() < 1 / 180.0 && this.spent_time >= 3 && !this.is_me)
        {
        ...
        else
        {
            ...
            {
                this.speed = this.playground.height * 0.05  / this.radius * this.playground.height * 0.55;
                let moved = Math.min(this.move_length, this.speed * this.timedelta / 1000);
                this.x += this.vx * moved;
                this.y += this.vy * moved;
                this.move_length -= moved;
...
class FireBall extends AcGameObject
{
    ...
    update()
    {
        ...
        for (let i = 0; i < this.playground.players.length; i ++ )
        {
            ...
            {
                if (!this.player.is_me && !player.is_me) continue;
                this.attack(player);
            }
        }
...
class AcGamePlayground
{
    constructor(root)
   {
    ...
    {  // 打开playground界面
        ...
        this.players = [];
        this.players.push(new Player(this, this.width / 2, this.height / 2, this.height * 0.05, "white", this.height * 0.7, true));

        for (let i = 0; i < 3; i ++ )
        {
            this.players.push(new Player(this, this.width / 2, this.height / 2, this.height * 0.05, this.get_random_color(), this.height * 0.7, false));
        }
